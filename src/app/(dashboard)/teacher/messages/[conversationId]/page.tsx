...
export default function TeacherDMPage() {
  ...
  const otherUser = (conversation as any)?.student
  ...
  return (
    <ChatThreadShell>
      ...
      <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-border bg-white shrink-0">
        ...
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-primary-light flex items-center justify-center text-white font-bold flex-shrink-0">
          {otherUser?.full_name?.charAt(0)?.toUpperCase() || 'S'}
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-text truncate">{otherUser?.full_name || 'Student'}</h3>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 sm:px-6 py-3 sm:py-4 space-y-3 bg-surface">
        {messages.length === 0 && (
          <div className="text-center py-12">
            <p className="text-text-light">No messages in this conversation yet</p>
          </div>
        )}
